...
import { getUploadLogs } from '@/app/(dashboard)/upload/actions'
...
export function UploadLogs() {
  ...
  useEffect(() => {
    async function fetchLogs() {
      try {
        const data = await getUploadLogs()
        setLogs(data)
      } catch (error) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Últimas 10 importaciones de archivos Excel</CardDescription>
      </CardHeader>
      <CardContent>
        {logs.length === 0 ? (
                    ...
                    <TableCell>
                      <div className="text-sm">
                        <p className="font-medium">
                          {log.uploaded_by_profile?.full_name || 'Usuario'}
                        </p>
                        <p className="text-muted-foreground text-xs">
                          {log.uploaded_by_profile?.email}
                        </p>
                      </div>
                    </TableCell>
